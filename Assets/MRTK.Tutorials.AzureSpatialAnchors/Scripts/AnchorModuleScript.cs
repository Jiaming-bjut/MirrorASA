...
public class AnchorModuleScript : MonoBehaviour
{
    ...
    {
        await CreateAzureAnchor(anchor);
    ...
    public void Client()
    {
        StartCoroutine(ClientPreparation());
    }
    IEnumerator ClientPreparation()
    {
        yield return GetAzureAnchorIdFromNetwork();
        FindAzureAnchor();
        manager.StartClient();
...
#region Public Methods
 ...
 Debug.Log("\nAnchorModuleScript.RemoveLocalAnchor()");
    ...
    IEnumerator GetAzureAnchorIdFromNetwork()
    {
        Debug.Log("\nAnchorModuleScript.GetSharedAzureAnchorID()");

        yield return StartCoroutine(GetSharedAzureAnchorIDCoroutine(publicSharingPin));
    }
#endregion
